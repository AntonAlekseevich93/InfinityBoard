...
import tooltip_area.TooltipAreaCustom
    ...
    Card(
        ...
        Row(
            ...
            AnimatedVisibility(visible = selectedViewsTypes.contains(ViewsType.LIST)) {
                TooltipAreaCustom(Strings.list, onClick = {
            ...
            AnimatedVisibility(visible = selectedViewsTypes.contains(ViewsType.KANBAN)) {
                TooltipAreaCustom(Strings.kanban, onClick = {
            ...
            AnimatedVisibility(visible = selectedViewsTypes.contains(ViewsType.CALENDAR)) {
                TooltipAreaCustom(Strings.calendar, onClick = {
            ...
            Box() {
                Box(
                    ...
                    ).defaultMinSize(minHeight = 26.dp).padding(start = 6.dp),
                    contentAlignment = Alignment.Center,
                ) {
                    TooltipAreaCustom(Strings.edit) {
                        Image(
                            painter = painterResource(Drawable.drawable_ic_plus),
...
@Composable
internal fun IconViewType(iconResName: String, isSelected: Boolean, onClick: () -> Unit) {
    val cardBackground = if (isSelected) {
        Color(0xFF4D4D50) //todo переделать токен цвет)
    } else {
        ApplicationTheme.colors.cardBackgroundDark //todo переделать токен цвет)
    }
